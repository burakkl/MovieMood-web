...
router.post('/get-or-create', async (req, res) => {
    try {
        ...
        if (!chat) {
            const result = await db.prepare('INSERT INTO chats (user1_id, user2_id) VALUES (?, ?)').run(user1Id, user2Id);
            chat = await db.prepare('SELECT * FROM chats WHERE chat_id = ?').get(result.lastID);
        }
...
router.post('/:chatId/messages', async (req, res) => {
    try {
        ...
        if (!senderId || !messageText) return res.status(400).json({ error: 'Sender ID and message text are required' });
